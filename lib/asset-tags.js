...
function buildOrphanAssetKey(assetId) {
  return `orphan:${assetId}`;
}
...
function buildAdminAssetKey(category, filename) {
  return `admin:${category}:${filename}`;
}
...
async function attachTagsToAdminAssets(grouped) {
  return attachTagsToGroupedAssets(grouped, (category, filename) =>
    buildAdminAssetKey(category, filename)
  );
}
...
async function listTagsForScope(prefix, { sort = 'popular' } = {}) {
  ...
  // Closed allowlist only — never interpolate untrusted sort keys into ORDER BY.
  const ORDER_BY = {
    popular: 'count DESC, tag ASC',
    recent: 'last_used_at DESC, tag ASC',
    alpha: 'tag ASC',
  };
  const orderClause = ORDER_BY[sort] || ORDER_BY.popular;
