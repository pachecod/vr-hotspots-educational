...
function isAllowedOnHostedOrigin(req) {
  ...
  if (p === '/' || p === '') return true; // Render health checks often probe /
  if (hostedPathPrefix(p)) return true;
  if (p === '/health' || p === '/healthz' || p === '/favicon.ico') return true;
  return false;
}
...
function createHostedOriginGuard() {
  return function hostedOriginGuard(req, res, next) {
    ...
    const p = (req.path || req.url || '').split('?')[0];
    // Do not serve the main app shell on the hosted origin root — only a health OK.
    // Render's deploy health check uses Host: hosted.… and path `/`.
    if (p === '/' || p === '') {
