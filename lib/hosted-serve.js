...
function resolveHostedRequestPath(req) {
  const pathOnly = String(req.path || '');
  if (pathOnly.startsWith('/hosted/')) return pathOnly;
...
function createHostedStaticHandler() {
  return async function hostedStaticHandler(req, res, next) {
    ...
    const parsed = parseHostedRequest(resolveHostedRequestPath(req));
