...
function legacyToInboxItem(submission) {
  return {
    ...
    featuredOnHostedGallery: submission.featuredOnHostedGallery,
...
function submissionIsHosted(sub) {
  return !!(sub.isHosted || sub.hostedPath || sub.hostedUrl);
}

function filterInboxItems(items, { filter, hostedFilter, featuredFilter } = {}) {
  let result = items;
  if (filter === 'with_notes') {
    result = result.filter((s) => s.studentNote && String(s.studentNote).trim());
  } else if (filter === 'without_notes') {
    result = result.filter((s) => !s.studentNote || !String(s.studentNote).trim());
  }
  if (hostedFilter === 'hosted') {
    result = result.filter((s) => submissionIsHosted(s));
  } else if (hostedFilter === 'not_hosted') {
    result = result.filter((s) => !submissionIsHosted(s));
  }
  if (featuredFilter === 'featured') {
    result = result.filter((s) => !!s.featuredOnHostedGallery);
...
async function mergeB2OrphansIntoInbox(
  dbInbox,
  b2Service,
  { filter, hostedFilter, featuredFilter, extraKnownKeys } = {}
) {
  // Inbox is submitted-only; also exclude repair/return/draft/assigned ZIPs already in DB.
  const knownKeys = new Set(dbInbox.map(submissionStorageKey));
  if (extraKnownKeys) {
    for (const k of extraKnownKeys) {
      if (k) knownKeys.add(String(k).replace(/^student-projects\//, ''));
    }
  }
  ...
  for (const b2File of b2Files) {
    ...
    if (knownKeys.has(b2File.fileName)) continue;
  ...
  orphans = filterInboxItems(orphans, { filter, hostedFilter, featuredFilter });

  const merged = filterInboxItems([...dbInbox, ...orphans], { filter, hostedFilter, featuredFilter });
  return merged.sort(
    (a, b) => new Date(b.submittedAt || 0).getTime() - new Date(a.submittedAt || 0).getTime()
  );
}

async function listLegacyInbox(b2Service, { filter, hostedFilter, featuredFilter } = {}) {
  const submissions = await syncSubmissionsWithB2(b2Service);
  let items = filterInboxItems(submissions.map(legacyToInboxItem), {
    filter,
    hostedFilter,
    featuredFilter,
  });
