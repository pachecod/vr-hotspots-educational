...
const THUMB_EXTENSIONS = ['.jpg', '.jpeg', '.png', '.webp', '.svg'];
...
/** B2 object key under the common-assets images category. */
function playgroundThumbRemotePath(slug, ext = '.jpg') {
  return buildRemotePath('images', playgroundThumbFilename(slug, ext));
}
...
function playgroundThumbServeUrl(slug) {
  return `/api/playground/thumbnails/${encodeURIComponent(slug)}`;
}

function playgroundThumbLookupPaths(slug) {
  const paths = [];
  for (const ext of THUMB_EXTENSIONS) {
    paths.push(playgroundThumbRemotePath(slug, ext));
  }
  for (const ext of THUMB_EXTENSIONS) {
...
async function uploadThumbnailFile(localPath, slug, contentType) {
  ...
  return playgroundThumbServeUrl(slug);
}

async function uploadCustomTemplateThumbnail(template, localPath, contentType) {
      ...
      : ext === '.webp'
        ...
        : ext === '.gif'
          ...
          : ext === '.svg'
            ? 'image/svg+xml'
            : 'image/jpeg');
  return uploadThumbnailFile(localPath, template.slug, mime);
}
