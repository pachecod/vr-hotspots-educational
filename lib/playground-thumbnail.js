...
function findManifestFile(manifest, name) {
  if (!Array.isArray(manifest)) return null;
  const entry = manifest.find((f) => f.name === name || f.id === name);
  return entry?.content != null ? String(entry.content) : null;
}

function extractImageFromHtml(html) {
  if (!html) return null;
  const dataMatch = html.match(/<img[^>]+src=["'](data:image\/[^"']+)["']/i);
  if (dataMatch) return { kind: 'data', value: dataMatch[1] };
  const urlMatch = html.match(/<img[^>]+src=["'](https?:\/\/[^"']+)["']/i);
  if (urlMatch) return { kind: 'url', value: urlMatch[1] };
  return null;
}

function extractImageFromCss(css) {
  if (!css) return null;
  const dataMatch = css.match(/url\(["']?(data:image\/[^"')]+)["']?\)/i);
  if (dataMatch) return { kind: 'data', value: dataMatch[1] };
  const urlMatch = css.match(/url\(["']?(https?:\/\/[^"')]+)["']?\)/i);
  if (urlMatch) return { kind: 'url', value: urlMatch[1] };
  return null;
}
...
async function generateFlatTemplateThumbnail(template) {
  ...
  const html = findManifestFile(manifest, 'index.html');
  const css = findManifestFile(manifest, 'style.css');
  const found = extractImageFromHtml(html) || extractImageFromCss(css);

  if (found?.kind === 'url') return found.value;
  if (found?.kind === 'data') {
    const parsed = dataUrlToBuffer(found.value);
    if (parsed) {
      const ext = parsed.mime.includes('png') ? '.png' : '.jpg';
      return uploadThumbnailBuffer(parsed.buffer, template.slug, parsed.mime, ext);
    }
  }
