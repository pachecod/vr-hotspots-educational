...
const DEV_PASSWORD_ENCRYPTION_KEY = 'vr-hotspots-dev-password-encryption-key';
...
function assertProductionSecrets() {
  ...
  const passwordEnc = process.env.STUDENT_PASSWORD_ENCRYPTION_SECRET || '';
  if (
    !passwordEnc ||
    passwordEnc === DEV_PASSWORD_ENCRYPTION_KEY ||
    passwordEnc === process.env.STUDENT_SESSION_SECRET ||
    passwordEnc === process.env.ADMIN_SESSION_SECRET
  ) {
    errors.push(
      'STUDENT_PASSWORD_ENCRYPTION_SECRET must be set to a unique random value (not a session secret) in production'
    );
  }
