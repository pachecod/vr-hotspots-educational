...
function renderGatePage({ returnTo = '/', error = '' } = {}) {
...
function sendGatePage(req, res, { status = 401, error = '' } = {}) {
  const returnTo = sanitizeReturnTo(req.originalUrl || '/', `${req.protocol}://${req.get('host')}`);
  res.status(status).type('html').send(renderGatePage({ returnTo, error }));
}
...
function registerSitePasswordRoutes(app) {
  ...
  app.get('/site-password', (req, res) => {
    ...
    const returnTo = sanitizeReturnTo(req.query.returnTo, `${req.protocol}://${req.get('host')}`);
    res.status(200).type('html').send(renderGatePage({ returnTo }));
  });
  ...
  app.post('/site-password', parseForm, (req, res) => {
    const body = req.body || {};
    ...
    if (!timingSafeEqualString(submitted, getSitePassword())) {
      return res
        .status(401)
        .type('html')
        .send(renderGatePage({ returnTo, error: 'Incorrect password. Try again.' }));
    }

    const helpers = getHelpers();
    const token = helpers.createToken({});
    helpers.setCookie(res, token);
    return res.redirect(returnTo || '/');
  });
}
