...
/**
 * Validate URL and resolve DNS once. Returns a pinned target so callers can
 * connect to `address` without a second DNS lookup (DNS rebinding defense).
 * @returns {Promise<{ url: URL, address: string, family: number, hostname: string }>}
 */
async function assertSafeOutboundUrl(rawUrl) {
  ...
  if (net.isIP(hostname)) {
    ...
    return { url: parsed, address: hostname, family, hostname };
  }
  ...
  if (isPrivateOrMetadataIp(chosen.address)) {
    throw new Error('Cannot fetch from private or local addresses.');
  }
  ...
  return {
    ...
    family: chosen.family || (net.isIPv6(chosen.address) ? 6 : 4),
    hostname,
  };
}
