...
async function loadProjects({ classId, studentId, q } = {}) {
  ...
  return rows.map((row) => {
    ...
    if (row.hosted_path && row.is_hosted) {
      try {
        const urls = resolveHostedProjectUrls(row.hosted_path);
...
async function loadSharedCommonAssets({ q } = {}) {
  if (!process.env.B2_KEY_ID) return [];
      ...
      items.push(
        itemBase({
          ...
          storageHints: {
            ...
            diskPaths: [],
            dbTables: ['asset_tags', 'site_assets'],
          },
...
async function loadAdminOnlyAssets({ q } = {}) {
  if (!process.env.B2_KEY_ID) return [];
  const { listAdminAssetFilesFromB2 } = require('../site-assets');
  const { attachTagsToAdminAssets } = require('../asset-tags');
  const grouped = await listAdminAssetFilesFromB2();
  await attachTagsToAdminAssets(grouped);
  const items = [];
  for (const [category, assets] of Object.entries(grouped)) {
    for (const asset of assets || []) {
      if (q && !String(asset.name).toLowerCase().includes(q.toLowerCase())) continue;
      items.push(
        itemBase({
          ...
          links: {
            ...
            flatUrl: null,
            downloadUrl: null,
            previewUrl: `/admin/admin-assets/${encodeURIComponent(category)}/${encodeURIComponent(asset.name)}`,
            qrUrl: null,
          },
          storageHints: {
            b2Paths: [asset.b2_path],
            diskPaths: [],
            dbTables: ['asset_tags', 'site_assets'],
          },
          size: Number(asset.size),
...
/** @deprecated use loadSharedCommonAssets */
async function loadCommonAssets(filters) {
  return loadSharedCommonAssets(filters);
}
...
async function loadAdminSiteContent(filters) {
  const { type } = filters;
  let items = [];
  if (!type || type === 'admin_asset') items = items.concat(await loadAdminOnlyAssets(filters));
  if (!type || type === 'common_asset') items = items.concat(await loadSharedCommonAssets(filters));
  return items;
}
...
async function buildContentInventory(filters = {}) {
  ...
  if (isAdminContentClassFilter(filters)) {
    ...
    if (!filters.studentId && (!type || type === 'admin_asset')) {
      items = items.concat(await loadAdminOnlyAssets({ q: filters.q }));
    }
    if (!filters.studentId && (!type || type === 'common_asset')) {
      items = items.concat(await loadSharedCommonAssets({ q: filters.q }));
    }
  }
