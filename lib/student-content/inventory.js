...
async function loadSnippets({ q } = {}) {
  const { listSnippets } = require('../snippets');
    ...
    .map((row) =>
      itemBase({
        ...
        links: {
          ...
          flatUrl: null,
          downloadUrl: null,
          previewUrl: 'admin-snippets.html',
          qrUrl: null,
        },
        storageHints: {
          b2Paths: [],
...
function shouldIncludeAdminSiteContent(filters) {
  if (isAdminContentClassFilter(filters)) return true;
  const { studentId, orphaned, type } = filters;
  if (studentId || orphaned) return false;
  if (!type) return true;
  return type === 'common_asset' || type === 'template' || type === 'snippet';
}
...
async function loadAdminSiteContent(filters) {
  ...
  if (!type || type === 'snippet') items = items.concat(await loadSnippets(filters));
  return items;
}
...
async function buildContentInventory(filters = {}) {
  ...
  if (isAdminContentClassFilter(filters)) {
    ...
    if (shouldIncludeAdminSiteContent(filters)) {
      items = items.concat(await loadAdminSiteContent(filters));
    }
  }
