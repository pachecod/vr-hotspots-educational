...
const { deleteHostedProject, listHostedProjectPaths } = require('../hosted-b2-storage');
...
async function removeHostedDir(hostedPath) {
...
async function purgeVrTour(studentId, slug) {
  ...
  if (hostedPath) {
    ...
    await removeHostedDir(hostedPath);
  }
...
async function purgeAllHostedProjects() {
  const manifest = { diskPaths: [], dbTables: [], unhostedCount: 0 };

  const paths = [...new Set(await listHostedProjectPaths())];
  for (const hostedPath of paths) {
    await removeHostedDir(hostedPath);
    manifest.diskPaths.push(hostedPath);
    manifest.unhostedCount++;
  }

  const logs = loadSubmissionsLog();
  let legacyUpdated = false;
  for (const sub of logs) {
    if (sub.hostedPath || sub.isHosted || sub.hostedUrl) {
      delete sub.hostedPath;
      delete sub.hostedUrl;
      delete sub.hostedAt;
      sub.isHosted = false;
      sub.featuredOnHostedGallery = false;
      legacyUpdated = true;
    }
  }
  if (legacyUpdated) {
  ...
  if (isDbEnabled()) {
    const { rowCount: versionCount } = await query(
      `UPDATE project_versions
       SET hosted_path = NULL, hosted_url = NULL, is_hosted = FALSE, hosted_at = NULL,
           featured_on_hosted_gallery = FALSE
       WHERE is_hosted = TRUE OR hosted_path IS NOT NULL`
    );
    if (versionCount > 0) manifest.dbTables.push('project_versions (hosting cleared)');

    const { rowCount: submissionCount } = await query(
      `UPDATE submissions
       SET hosted_path = NULL, hosted_url = NULL, is_hosted = FALSE, hosted_at = NULL,
           featured_on_hosted_gallery = FALSE
       WHERE is_hosted = TRUE OR hosted_path IS NOT NULL`
    );
    if (submissionCount > 0) manifest.dbTables.push('submissions (hosting cleared)');

    const { rowCount: flatCount } = await query(
      `UPDATE flat_page_projects
       SET hosted_path = NULL, hosted_url = NULL, is_hosted = FALSE, hosted_at = NULL
       WHERE is_hosted = TRUE OR hosted_path IS NOT NULL`
    );
    if (flatCount > 0) manifest.dbTables.push('flat_page_projects (hosting cleared)');

    const { rowCount: tourCount } = await query(
      `DELETE FROM student_published_tours WHERE hosted_path IS NOT NULL`
    );
    if (tourCount > 0) manifest.dbTables.push('student_published_tours (removed)');
  }
...
async function purgeHostedSubmission({ versionId, fileName } = {}) {
  ...
  if (versionId && isDbEnabled()) {
    ...
    if (version && version.hostedPath) {
      ...
      await removeHostedDir(version.hostedPath);
      await projectVersionsDb.updateVersionHosting(version.id, {
        hostedPath: null,
        hostedUrl: null,
        isHosted: false,
      });
      await projectVersionsDb.updateVersionGalleryFeature(version.id, false);
  ...
  if (fileName) {
    ...
    if (isDbEnabled()) {
      ...
      await submissionsDb.updateSubmissionGalleryFeature(fileName, false);
...
async function purgeAdminAsset(category, filename) {
  const manifest = { b2Paths: [], diskPaths: [], dbTables: ['asset_tags', 'site_assets'] };
  ...
  await deleteTagsForKey(buildAdminAssetKey(category, filename));
  const { deleteSiteAssetRecord } = require('../site-assets');
...
async function purgeStudentProjectsAndPages(studentId) {
  ...
  const hostedPaths = await listHostedProjectPaths();
  for (const entry of hostedPaths) {
    if (prefixes.some((p) => entry.startsWith(p))) {
      await removeHostedDir(entry);
    }
  }
...
async function describePurge({ type, id, studentId, slug, category, filename, fileName, versionId }) {
  ...
  switch (type) {
    ...
    case 'common_asset': {
      ...
      manifest.willRemove.dbTables.push('asset_tags', 'site_assets', 'common-assets (B2)');
      break;
    }
    case 'admin_asset': {
      const parsed = parseCommonAssetId(id, category, filename);
      if (parsed.category && parsed.filename) {
        manifest.willRemove.b2Paths.push(
          buildAdminRemotePath(parsed.category, parsed.filename)
        );
      }
      manifest.willRemove.dbTables.push('asset_tags', 'site_assets', 'admin-assets (B2)');
      break;
    }
    case 'template':
...
async function purgeContentItem({ type, id, studentId, slug, category, filename, fileName, versionId }) {
  switch (type) {
    ...
    case 'admin_asset': {
      const parsed = parseCommonAssetId(id, category, filename);
      return purgeAdminAsset(parsed.category, parsed.filename);
    }
    case 'template':
...
module.exports = {
  ...
  isB2PathPurged,
  removeHostedDir,
  getHostedDir: require('../hosted-b2-storage').getHostedDir,
};
