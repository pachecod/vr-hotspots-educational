...
class MuseumRuntime {
  ...
  createInfoDisplay() {
    ...
    panel.setAttribute('material', 'src: ' + info.panel.images[0] + '; side: double; shader: flat');
  ...
  setupSlideshow() {
    ...
    const update = () => {
      panel.setAttribute('material', 'src: ' + images[this.currentImageIndex] + '; side: double; shader: flat');
  ...
  trackLoading() {
    ...
    const bump = () => {
      ...
      if (loaded >= total) setTimeout(() => overlay.classList.add('is-hidden'), 400);
    };

    if (assets) {
      const items = assets.querySelectorAll('*');
      total = Math.max(items.length, 1);
      items.forEach((item) => {
        if (item.hasLoaded) bump();
        else {
          item.addEventListener('loaded', bump, { once: true });
          item.addEventListener('error', bump, { once: true });
        }
      });
    } else {
      total = 1;
    ...
    setTimeout(() => overlay.classList.add('is-hidden'), 12000);
  }
}
