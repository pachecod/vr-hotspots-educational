...
function renderStudentLoginGate(containerId, onAuthenticated) {
        ...
        ${students.length
          ...
          <button type="button" class="student-name-btn" data-id="${s.id}" style="
            ...
            ${escapeHtml(s.display_name)}
              ...
              .join('')
          : '<p style="color:#aaa;">No students in this class yet.</p>'}
      </div>
    `;
  ...
  function renderPasswordStep() {
    ...
    stepEl.innerHTML = `
      <button type="button" id="student-back-student" style="background:none;border:none;color:#4caf50;cursor:pointer;margin-bottom:12px;padding:0;">← Back to names</button>
      <div style="background:#333;border:1px solid #555;border-radius:6px;padding:12px;margin-bottom:16px;">
        <div style="color:#888;font-size:12px;">Class</div>
        <div style="font-weight:bold;">${escapeHtml(selectedClass.name)}</div>
        <div style="color:#888;font-size:12px;margin-top:8px;">Student</div>
        <div style="font-weight:bold;">${escapeHtml(selectedStudent.display_name)}</div>
      </div>
      <label style="display:block;color:#ccc;margin-bottom:6px;font-size:13px;">Password from your teacher</label>
